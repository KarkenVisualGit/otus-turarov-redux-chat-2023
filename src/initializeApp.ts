...
export const initializeApp = () => {
    ...
    if (emojiPickerButton && emojiPicker) {
        emojiPickerButton.addEventListener('click', () => toggleEmojiPicker(emojiPicker));
    }

    if (emojiPicker && messageInput) {
        emojiPicker.addEventListener('click', (e: Event) => {
            const target = e.target as HTMLElement;
            if (target && target.classList.contains('emoji') && target instanceof HTMLImageElement) {
                insertEmoji(target, messageInput);
            }
        });
    }
    ...
    if (messageForm) {
        messageForm.addEventListener('submit', (e) => handleSubmit(e, messageInput, nicknameInput));
    }
    ...
    store.subscribe(() => {
        ...
        state.chat.messages.forEach((message) => {
            if (!renderedMessageIds.has(message.id)) {
                addMessageToDOM(message, messagesContainer, handleDeleteMessage, renderedMessageIds);
            }
        });
